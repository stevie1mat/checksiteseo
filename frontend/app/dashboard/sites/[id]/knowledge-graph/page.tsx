...
export default async function KnowledgeGraphPage({ params }: { params: { id: string } }) {
    ...
    const initialKg = breakdown.authority?.knowledge_graph?.data || { primary_entity: 'Not found', relationships: {}, missing_critical: [] };
    ...
    return (
        <RealtimeMetricView
            ...
            transform={(report: AEOReport) => {
                // report.authority might be 'Analysis' string or object.
                // But from API route it should be structured if complete.
                // However, TS definition says authority is string | ...
                // We need to cast or check.
                // Actually report.knowledgeGraph is top level in AEOReport.
                // API route: knowledgeGraph: { primaryEntity: ... }
                // Use the top level one for convenience as API route constructs it.
                // Or fallback to checking authority object if that's preferred.

                // Let's use the top level `knowledgeGraph` from AEOReport which matches our API response
                const kg = report.knowledgeGraph || {};

                return {
                    primary_entity: kg.primaryEntity || 'Not found',
                    relationships: kg.relationships || {},
                    missing_critical: kg.missing_critical || []
                };
            }}
        >
            {(kg: any) => {
